...
describe("Get User Favorite Places Usecase", () => {
    let app: Elysia;
    ...
    beforeAll(async () => {
        //@ts-expect-error
        app = new Elysia().use(authMiddleware).use(AuthController).use(UserController);
        ...
        // 4. Adiciona local aos favoritos
        await db.insert(favoritePlacesSchema).values({ userId, placeId }).returning();
    });
    ...
    it("should add a new favorite place successfully", async () => {
        ...
        const response = await app.handle(
            new Request("http://localhost:8080/users/favorite-places", {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Cookie: tokenData,
